    ...
    <div className="container-fluid p-4 p-lg-5">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3 mb-4">
        <div>
          ...
          <p className="text-secondary mb-0">Update product details, colors, and images.</p>
        </div>
        <a className="btn btn-outline-secondary" href="/admin/products">Back to products</a>
      </div>

      {searchParams?.error && <div className="alert alert-danger" role="alert">{searchParams.error}</div>}

      <form action={updateProductAction} method="post" encType="multipart/form-data" className="bg-white border rounded p-4">
        ...
        <div className="row g-4">
          <div className="col-lg-8">
            ...
            <div className="mb-3">
              ...
              <input className="form-control" type="text" id="name" name="name" maxLength={150} defaultValue={product.name} required />
            </div>
            <div className="row g-3">
              <div className="col-md-6">
                <label className="form-label" htmlFor="sku">SKU</label>
                <input className="form-control" type="text" id="sku" name="sku" maxLength={100} defaultValue={product.sku} required />
              </div>
              <div className="col-md-6">
                <label className="form-label" htmlFor="category">Category</label>
                <input className="form-control" type="text" id="category" name="category" maxLength={100} defaultValue={product.category} required />
              </div>
            </div>
            <div className="mb-3 mt-3">
              <label className="form-label" htmlFor="shortDescription">Short description</label>
              <textarea className="form-control" id="shortDescription" name="shortDescription" rows={3} maxLength={255} defaultValue={product.shortDescription ?? ''} required />
            </div>
            <div className="mb-3">
              <label className="form-label" htmlFor="fullDescription">Full description</label>
              <textarea className="form-control" id="fullDescription" name="fullDescription" rows={7} defaultValue={product.fullDescription ?? ''} required />
            </div>
            <div className="mb-3">
              <label className="form-label" htmlFor="mainImage">Add new primary image</label>
            ...
            <div className="mb-3">
              <label className="form-label" htmlFor="galleryImages">Add gallery images</label>
          ...
          <div className="col-lg-4">
            ...
            <div className="mb-3">
              ...
              <input className="form-control" type="number" id="price" name="price" min="0" step="0.01" defaultValue={Number(product.price.toString())} required />
            </div>
            <div className="mb-3">
              <label className="form-label" htmlFor="salePrice">Sale price (PKR)</label>
              <input className="form-control" type="number" id="salePrice" name="salePrice" min="0" step="0.01" defaultValue={product.salePrice ? Number(product.salePrice.toString()) : ''} />
            </div>
            <div className="mb-3">
              <label className="form-label" htmlFor="stockQuantity">Stock quantity</label>
              <input className="form-control" type="number" id="stockQuantity" name="stockQuantity" min="0" step="1" defaultValue={product.stockQuantity} required />
            </div>
            <div className="mb-3">
              <label className="form-label" htmlFor="status">Status</label>
              <select className="form-select" id="status" name="status" defaultValue={product.isActive ? 'active' : 'inactive'}>
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
              </select>
            </div>
            <div className="form-check mt-4">
              <input className="form-check-input" type="checkbox" id="isFeatured" name="isFeatured" value="1" defaultChecked={product.isFeatured} />
        ...
        <div className="d-flex justify-content-between align-items-center gap-2 mb-3">
          <div>
            ...
            <p className="text-secondary small mb-0">Update colors, price adjustments, stock, and status.</p>
          </div>
